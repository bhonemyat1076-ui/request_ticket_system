<div class="flex flex-col h-full overflow-y-auto py-4">
    <nav class="flex-1 px-4 space-y-2">
           ...
           <x-sidebar-link :href="route('tickets.create')" :active="request()->routeIs('tickets.create')">
            ...
            <div x-data="{ open: true }" class="mt-2">
                ...
                <div x-show="open" 
                    ...
                    <x-sidebar-link href="{{ route('user.all-tickets.show') }}" :active="request()->routeIs('user.all-tickets.show')" class="py-2 text-xs">
                         {{ __('All Tickets') }}    
                    </x-sidebar-link>

                    <x-sidebar-link href="{{ route('user.open-tickets.show') }}" :active="request()->routeIs('user.open-tickets.show')" class="py-2 text-xs">
                         {{ __('Open Tickets') }}
                    </x-sidebar-link>

                    <x-sidebar-link href="{{ route('user.pending-tickets.show') }}" :active="request()->routeIs('user.pending-tickets.show')" class="py-2 text-xs">
                         {{ __('Pending Tickets') }}
                    </x-sidebar-link>

                    <x-sidebar-link href="{{ route('user.success-tickets.show') }}" :active="request()->routeIs('user.success-tickets.show')" class="py-2 text-xs">
                         {{ __('Success Tickets') }}
                    </x-sidebar-link>

                    <x-sidebar-link href="{{ route('user.tickets.history') }}" :active="request()->routeIs('user.tickets.history')" class="py-2 text-xs">
                         {{ __('Ticket History') }}
                    </x-sidebar-link>
                </div>
            </div>
        ...
        @if(Auth::user()->is_admin)
            ...
            <x-sidebar-link href="{{ route('admin.tickets.open') }}" :active="request()->routeIs('admin.tickets.open')">
                <i class="fas fa-flag mr-2 text-red-600"></i> {{ __('Open Tickets') }}
            </x-sidebar-link>

            <x-sidebar-link href="{{ route('admin.tickets.pending') }}" :active="request()->routeIs('admin.tickets.pending')">
                <i class="fas fa-clock mr-2 text-red-600"></i> {{ __('Pending Tickets') }}
            </x-sidebar-link>

            <x-sidebar-link href="{{ route('admin.tickets.success') }}" :active="request()->routeIs('admin.tickets.success')">
                <i class="fas fa-check-circle mr-2 text-red-600"></i> {{ __('Success Tickets') }}
            </x-sidebar-link>
            
            <x-sidebar-link href="{{ route('admin.tickets.history') }}" :active="request()->routeIs('admin.tickets.history')">
