    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-6">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
            <div class="p-6 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <span class="text-sm font-bold text-gray-700 uppercase tracking-wider">Older than 1 month</span>
                <span class="text-xs text-gray-500">Showing {{ $historyTickets->firstItem() }} - {{ $historyTickets->lastItem() }} of {{ $historyTickets->total() }}</span>
            </div>
                            ...
                            <tr class="hover:bg-red-50/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $statusStyle = match($ticket->status) {
                                            ...
                                            'resolved', 'success' => 'bg-green-100 text-green-700 border-green-200',
                                            default => 'bg-gray-100 text-gray-700 border-gray-200',
                                        };
                                    @endphp
                                    <span class="px-3 py-1 text-[10px] font-black rounded-full border {{ $statusStyle }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('admin.tickets.show', $ticket) }}" 
                                       class="inline-flex items-center px-3 py-1.5 bg-white border border-red-600 text-red-600 text-xs font-bold rounded-md hover:bg-red-600 hover:text-white transition shadow-sm">
                                        Manage
                                    </a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="text-gray-400 italic">No success history tickets older than 1 month.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
